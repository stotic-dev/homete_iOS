...
現在 `homete/` 配下にフラットに配置されているコードを、Swift Package Manager（SPM）のローカルパッケージとして **Feature ベースの複数モジュール** に分割する。ビルド時間の短縮・モジュール境界の明確化・Preview の軽量化・テスタビリティの向上を目的とする。
...
- Feature 単体での Preview ビルドができず、プレビューが重い
