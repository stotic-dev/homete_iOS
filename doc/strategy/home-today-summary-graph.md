...
### 4. ホームの見た目の調整（動作確認時のフィードバック）

グラフが入ってホームが縦に長くなり、セクションの境目が分かりにくくなったため、あわせて次を調整した。

- 「今日の家事サマリー」「月の家事貢献度サマリー」「テンプレート設定の案内」をカードで区切る。共通のモディファイア `sectionCardStyle()`（`HometeUI/DesignSystem/Card/`）を追加した。`subSurface` で塗り、角丸16、影で境界を出す（ライトモードでは画面背景と同じ白系なので影が要る）
- 未完了の家事リストに「未完了の家事」の見出しと、未完了の総件数を付ける（表示は最大4件のため）
- 月間サマリーのタイトルを `headLineS` から `headLineM` にし、今日のサマリーと揃える
