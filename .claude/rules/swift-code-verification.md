...
**Swiftのコードを変更した場合、ユニットテストは省略不可。必ず実行すること。**

**流す前に影響範囲で要否を見極める。** テストコードのみの変更なら手順を絞れるし、Swiftのビルドに影響しない変更（ツール設定・ドキュメント・Functionsなど）ではビルド・テスト自体が不要。その場合は変更したもの自体の動作だけを最小限で確かめる。判断表はスキル参照。
